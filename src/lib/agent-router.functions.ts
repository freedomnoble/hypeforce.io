...
export const invokeAgentRouter = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Determine which agents should reply. (Membership lookups still use the
    // user-scoped client so RLS guards against cross-workspace probing.)
    ...
    // Single batched RPC fetches: agents, overrides, counters, recent
    // messages, brand voice, knowledge base, pinned files, project memos,
    // workspace agent + human roster, and the caller's BYOK keys. Also
    // validates that channel_id / dm_id belong to workspace_id.
    const ctx = await loadRouterContext({
    ...
    const kbBlock =
      kb.length > 0
        ? `\n\nWorkspace knowledge:\n${kb
            .map((k) => `- ${k.title}: ${(k.body ?? "").slice(0, 400)}`)
            .join("\n")}`
        : "";
